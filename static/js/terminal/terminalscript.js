            ...
            if (!allowedCommands.has(command)) {
                ...
                switch (command) {
                    ...
                    case 'help':
                        ...
                        break;
                    case 'play':
                        if (arg.toLowerCase() === 'invaders') {
                             appendOutputLine("\n:: Initiating Invader Defense Simulation... Launching module... ::");
                             window.open('/simulation/invaders', '_blank');
                        } else {
                             appendOutputLine(`Error: Unknown simulation module '${arg.replace(/</g, "&lt;").replace(/>/g, "&gt;")}'. Available: 'invaders'`);
                        }
                        break;
                    case 'cd':
                        handleCd(arg);
    ...
    function displayFileContent(filePathArg) {
        ...
        if (found && fileContent !== undefined) {
            ...
            if (isAudioLog) {
                ...
                const audioSrc = `/static/assets/audio/${lookupKey.slice(0, -4)}`;
    ...
    // --- showHelp Function ---
     function showHelp() {
        ...
        sortedCommands.forEach(cmd => {
            // Hide aliases from help to keep it clean
            if (cmd === 'list' || cmd === 'cat' || cmd === 'run_simulation') return;

            if (descriptions[cmd]) {
                 appendOutputLine(descriptions[cmd], false, false);
            } else {
